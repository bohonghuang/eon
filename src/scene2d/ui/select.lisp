...
(defmethod scene2d-layout ((entry select-box-border-entry))
  ...
  (let ((position +vector2-zeros+)
    ...
    (raylib:copy-rectangle (scene2d-bound entry) border)
    (incf (raylib:rectangle-x border) (- (raylib:vector2-x position) +select-box-border-entry-rectangle-padding+))
    (incf (raylib:rectangle-y border) (- (raylib:vector2-y position) +select-box-border-entry-rectangle-padding+))
    (incf (raylib:rectangle-width border) (* 2.0 +select-box-border-entry-rectangle-padding+))
    (incf (raylib:rectangle-height border) (* 2.0 +select-box-border-entry-rectangle-padding+))))
