...
(defmethod scene2d-construct-form ((type (eql 'scene2d-box)) &rest args &key children orientation (alignment :center) &allow-other-keys)
  (declare (ignore orientation))
  (remove-from-plistf args :children)
  (remove-from-plistf args :alignment)
  (with-gensyms (box)
    `(let ((,box (make-scene2d-box . ,args)))
       ,(scene2d-construct-children-form children (lambda (child) `(scene2d-box-add-child ,box ,child ,alignment)))
       ,box)))
