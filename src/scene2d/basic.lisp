...
(defmethod scene2d-draw ((scissor scene2d-scissor) position origin scale rotation tint)
  (clet* ((matrix (foreign-alloca '(:struct raylib:matrix)))
          (vectors (foreign-alloca '(:array (:struct raylib:vector2) 2)))
          (position-start (& ([] vectors 0)))
          (position-end (& ([] vectors 1))))
    (rlgl:%get-matrix-transform matrix)
    (setf (-> position-start raylib:x) (raylib:vector2-x position)
          (-> position-start raylib:y) (raylib:vector2-y position)
          (-> position-end raylib:x) (+ (raylib:vector2-x position) (raylib:vector2-x (scene2d-scissor-size scissor)))
          (-> position-end raylib:y) (+ (raylib:vector2-y position) (raylib:vector2-y (scene2d-scissor-size scissor))))
    (raylib:%vector2-transform position-start position-start matrix)
    (raylib:%vector2-transform position-end position-end matrix)
    (raylib:with-scissor-mode ((round (-> position-start raylib:x))
                               (round (-> position-start raylib:y))
                               (round (- (-> position-end raylib:x) (-> position-start raylib:x)))
                               (round (- (-> position-end raylib:y) (-> position-start raylib:y))))
      (call-next-method))))
